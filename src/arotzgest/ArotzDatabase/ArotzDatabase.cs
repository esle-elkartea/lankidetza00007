...
class ArotzDatabase {
  ...
  static string fileName = "ArotzGest.mdb";
  static string connectionString = "Provider = Microsoft.Jet.OleDb.4.0; Data Source = " + System.Windows.Forms.Application.StartupPath + "\\" + fileName;
  static void Main (string [] args) {
    ...
    new OleDbCommand ("create table Budgets (Id identity primary key, [Number] int not null, ClientId int not null, [Date] date not null, State int not null)", connection).ExecuteNonQuery ();
    ...
    new OleDbCommand ("create table Invoices (Id identity primary key, [Number] int not null, ClientId int not null, [Date] date not null, VAT int not null, State int not null)", connection).ExecuteNonQuery ();
    ...
    if (args.Length == 0 || args [0] != "/silent") MessageBox.Show ("La base de datos se ha generado con éxito", name, MessageBoxButtons.OK, MessageBoxIcon.Information);
  }
}
