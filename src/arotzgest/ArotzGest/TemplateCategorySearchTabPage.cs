...
class TemplateCategorySearchTabPage : TabPage {
  Button acceptButton, cancelButton, createButton, detailButton, searchButton;
  ListView itemListView;
  TextBox nameTextBox;
  public TemplateCategorySearchTabPage () {
    Name = "templateCategorySearch";
    Text = "Categorías de plantilla";
    acceptButton = Interface.Button_Create (acceptButton_Click);
    Interface.HeaderPanel_Create (this, 0, 8, "TemplateCategory", Text, "Buscar");
  ...
  void createButton_Click (object o, EventArgs e) {
    (TopLevelControl as MainForm).TemplateCategoryDetailTabPage_Open (0);
  }
  void detailButton_Click (object o, EventArgs e) {
    (TopLevelControl as MainForm).TemplateCategoryDetailTabPage_Open ((int) itemListView.SelectedItems [0].Tag);
  }
  ...
  void searchButton_Click (object o, EventArgs e) {
    ...
    foreach (Database.TemplateCategory item in Database.TemplateCategory.Search (nameTextBox.Text)) Interface.ListView_AddListViewItem (itemListView, new string [] { item.Name }, item.Id);
