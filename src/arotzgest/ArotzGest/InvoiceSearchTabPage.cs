...
class InvoiceSearchTabPage : TabPage {
  ...
  void searchButton_Click (object o, EventArgs e) {
    ...
    if (year > 0) year += 2000;
    foreach (Database.Invoice item in Database.Invoice.Search (clientId, monthComboBox.SelectedIndex, year, stateComboBox.SelectedIndex - 1)) Interface.ListView_AddListViewItem (itemListView, new string [] { item.Number + "/" + item.Date.Year, item.ClientName, Data.Date_Format (item.Date), Database.InvoiceStates [item.State] }, item.Id);
