...
class ProviderDetailTabPage : TabPage {
  ...
  public ProviderDetailTabPage (int id) {
    ...
    if (id > 0) item = Database.Provider.Get (id);
  ...
  void deleteButton_Click (object o, EventArgs e) {
    if (!Interface.ConfirmationDialog_Show ("Está seguro de que desea borrar el proveedor actual?")) return;
    if (item.CheckUsed ()) {
      Interface.ErrorDialog_Show ("El proveedor actual no se puede borrar por que tiene entidades relacionadas");
      return;
    }
    Database.Provider.Delete (item.Id);
  ...
  void modifyButton_Click (object o, EventArgs e) {
    if (modifying) {
      ...
      else if (!Database.Provider.CheckUnique (item, nameTextBox.Text)) errorManager.Add (nameTextBox, "Ya existe un proveedor con el nombre especificado");
      if (errorManager.Controls.Count > 0) {
        errorManager.Controls [0].Focus ();
        return;
      }
      Database.Provider criteria = new Database.Provider (nameTextBox.Text, ficTextBox.Text, addressTextBox.Text, cityTextBox.Text, provinceTextBox.Text, zipTextBox.Text, phoneTextBox.Text, mobileTextBox.Text, emailTextBox.Text, homepageTextBox.Text, accountTextBox.Text, notesTextBox.Text);
      item = item == null ? Database.Provider.Create (criteria) : item.Update (criteria);
    }
  ...
  void setModifying (bool value) {
    ...
    Name = "providerDetail_" + (item == null ? 0 : item.Id);
    Text = "Proveedor: " + (item == null ? "" : item.Name);
