...
class ClientSearchTabPage : TabPage {
  ...
  void createButton_Click (object o, EventArgs e) {
    (TopLevelControl as MainForm).ClientDetailTabPage_Open (0);
  }
  void detailButton_Click (object o, EventArgs e) {
    (TopLevelControl as MainForm).ClientDetailTabPage_Open ((int) itemListView.SelectedItems [0].Tag);
  }
  ...
  void searchButton_Click (object o, EventArgs e) {
    ...
    foreach (Database.Client item in Database.Client.Search (nameTextBox.Text)) Interface.ListView_AddListViewItem (itemListView, new string [] { item.Name, item.FIC, item.Phone }, item.Id);
