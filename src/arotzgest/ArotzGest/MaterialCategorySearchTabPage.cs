...
class MaterialCategorySearchTabPage : TabPage {
  Button acceptButton, cancelButton, createButton, detailButton, searchButton;
  ListView itemListView;
  TextBox nameTextBox;
  public MaterialCategorySearchTabPage () {
    Name = "materialCategorySearch";
    Text = "Categorías de material";
    acceptButton = Interface.Button_Create (acceptButton_Click);
    Interface.HeaderPanel_Create (this, 0, 8, "MaterialCategory", Text, "Buscar");
  ...
  void createButton_Click (object o, EventArgs e) {
    (TopLevelControl as MainForm).MaterialCategoryDetailTabPage_Open (0);
  }
  void detailButton_Click (object o, EventArgs e) {
    (TopLevelControl as MainForm).MaterialCategoryDetailTabPage_Open ((int) itemListView.SelectedItems [0].Tag);
  }
  ...
  void searchButton_Click (object o, EventArgs e) {
    ...
    foreach (Database.MaterialCategory item in Database.MaterialCategory.Search (nameTextBox.Text)) Interface.ListView_AddListViewItem (itemListView, new string [] { item.Name }, item.Id);
