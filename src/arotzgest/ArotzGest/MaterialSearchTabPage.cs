...
class MaterialSearchTabPage : TabPage {
  Button acceptButton, cancelButton, createButton, detailButton, searchButton;
  ComboBox categoryComboBox, providerComboBox;
  ListView itemListView;
  TextBox nameTextBox;
  public MaterialSearchTabPage () {
    Name = "materialSearch";
    Text = "Materiales";
    acceptButton = Interface.Button_Create (acceptButton_Click);
    Interface.HeaderPanel_Create (this, 0, 8, "Material", Text, "Buscar");
    ...
    foreach (Database.MaterialCategory category in Database.MaterialCategory.List ()) Interface.ComboBox_AddItem (categoryComboBox, category, false);
    Interface.Label_Create (this, 0, 76, "Nombre");
    nameTextBox = Interface.TextBox_Create (this, 128, 76, 128, 1, 255);
    Interface.Label_Create (this, 0, 104, "Proveeedor");
    providerComboBox = Interface.ComboBox_Create (this, 128, 104, 128);
    Interface.ComboBox_AddItem (providerComboBox, "", true);
    foreach (Database.Provider provider in Database.Provider.List ()) Interface.ComboBox_AddItem (providerComboBox, provider, false);
    ...
    Interface.ListView_AddColumnHeader (itemListView, 128, "Proveedor");
  ...
  protected override void OnGotFocus (EventArgs e) {
    ...
    (itemListView.Items.Count == 0 ? categoryComboBox as Control : itemListView).Focus ();
  }
  ...
  void createButton_Click (object o, EventArgs e) {
    (TopLevelControl as MainForm).MaterialDetailTabPage_Open (0);
  }
  void detailButton_Click (object o, EventArgs e) {
    (TopLevelControl as MainForm).MaterialDetailTabPage_Open ((int) itemListView.SelectedItems [0].Tag);
  }
  ...
  void searchButton_Click (object o, EventArgs e) {
    ...
    int categoryId = 0;
    if (categoryComboBox.SelectedIndex > 0) categoryId = (categoryComboBox.SelectedItem as Database.MaterialCategory).Id;
    int providerId = 0;
    if (providerComboBox.SelectedIndex > 0) providerId = (providerComboBox.SelectedItem as Database.Provider).Id;
    foreach (Database.Material item in Database.Material.Search (nameTextBox.Text, categoryId, providerId)) Interface.ListView_AddListViewItem (itemListView, new string [] { item.CategoryName, item.Name, item.ProviderName }, item.Id);
