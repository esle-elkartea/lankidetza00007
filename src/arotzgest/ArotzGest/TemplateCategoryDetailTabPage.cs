...
class TemplateCategoryDetailTabPage : TabPage {
  ...
  public TemplateCategoryDetailTabPage (int id) {
    ...
    if (id > 0) item = Database.TemplateCategory.Get (id);
  ...
  void deleteButton_Click (object o, EventArgs e) {
    if (!Interface.ConfirmationDialog_Show ("Está seguro de que desea borrar la categoría de plantilla actual?")) return;
    if (item.CheckUsed ()) {
      Interface.ErrorDialog_Show ("La categoría de plantilla actual no se puede borrar por que tiene entidades relacionadas");
      return;
    }
    Database.TemplateCategory.Delete (item.Id);
  ...
  void modifyButton_Click (object o, EventArgs e) {
    if (modifying) {
      ...
      else if (!Database.TemplateCategory.CheckUnique (item, nameTextBox.Text)) errorManager.Add (nameTextBox, "Ya existe una categoría de plantilla con el nombre especificado");
      if (errorManager.Controls.Count > 0) {
        errorManager.Controls [0].Focus ();
        return;
      }
      Database.TemplateCategory criteria = new Database.TemplateCategory (nameTextBox.Text);
      item = item == null ? Database.TemplateCategory.Create (criteria) : item.Update (criteria);
    }
  ...
  void setModifying (bool value) {
    ...
    Name = "templateCategoryDetail_" + (item == null ? 0 : item.Id);
    Text = "Categoría de plantilla: " + (item == null ? "" : item.Name);
