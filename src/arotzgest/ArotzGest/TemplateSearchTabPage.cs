...
class TemplateSearchTabPage : TabPage {
  ...
  public TemplateSearchTabPage () {
    ...
    foreach (Database.TemplateCategory category in Database.TemplateCategory.List ()) Interface.ComboBox_AddItem (categoryComboBox, category, false);
  ...
  void createButton_Click (object o, EventArgs e) {
    (TopLevelControl as MainForm).TemplateDetailTabPage_Open (0);
  }
  void detailButton_Click (object o, EventArgs e) {
    (TopLevelControl as MainForm).TemplateDetailTabPage_Open ((int) itemListView.SelectedItems [0].Tag);
  }
  ...
  void searchButton_Click (object o, EventArgs e) {
    ...
    int categoryId = 0;
    if (categoryComboBox.SelectedIndex > 0) categoryId = (categoryComboBox.SelectedItem as Database.TemplateCategory).Id;
    foreach (Database.Template item in Database.Template.Search (nameTextBox.Text, categoryId)) Interface.ListView_AddListViewItem (itemListView, new string [] { item.Name, item.CategoryName }, item.Id);
